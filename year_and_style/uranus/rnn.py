...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) > max_length:  # truncated
            sentence = sentence[:max_length]
            labels = labels[:max_length]
            mask = [True for i in sentence]
        else:
            mask = [True for i in sentence] + [False for i in range(max_length - len(sentence))]
            sentence = sentence + [zero_vector for i in range(max_length - len(sentence))]
            labels = labels + [zero_label for i in range(max_length - len(labels))]
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(shape=[None, self.max_length, Config.n_features], dtype=tf.int32)
    ...
    def add_embedding(self):
        ...
        Returns:
            Michael - This is a TYPO!!!!. Suppose to be (None, max_length, n_features * embedding_size)
            embeddings: tf.Tensor of shape (None, n_features*embed_size)
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings = tf.Variable(self.pretrained_embeddings)
        look_up = tf.nn.embedding_lookup(params=embeddings, ids=self.input_placeholder)

        embeddings = tf.reshape(look_up, [-1, self.max_length, Config.n_features * self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        Returns:
            Michael: This is wrong. It's suppose to be (None, max_length, n_classes)
    ...
    def add_loss_op(self, preds):
        ...
        masked = tf.boolean_mask(losses, self.mask_placeholder)
